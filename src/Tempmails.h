...
class Tempmails {
private:
    std::string api_base = "https://www.tempmails.cc/api";
...
public:
    ...
    pplx::task<json::value> generate_email() {
        return make_api_call("/mailboxes", "POST","{}");
    }

    pplx::task<json::value> get_mails(const std::string& email) {
        return make_api_call("/mailboxes/" + utility::conversions::to_utf8string(web::uri::encode_data_string(utility::conversions::to_string_t(email))), "GET");
    }

    pplx::task<json::value> delete_email(const std::string& email) {
        return make_api_call("/mailboxes/" + utility::conversions::to_utf8string(web::uri::encode_data_string(utility::conversions::to_string_t(email))), "DEL");
    }
};
